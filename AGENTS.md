...
- **`docgen/`** — `codebase.py` (deepagents pipeline + repo tools), `spec.py` (skeleton + LangGraph enrichment), `database.py` (MCP introspection RE flow), `verification.py` (citations, LLM judge `DOCGEN_JUDGE_ENABLED`, Mermaid verify/repair, provenance diff, DSN masking), `jobs.py` (async 202+poll, `DOCGEN_MAX_WORKERS`), `summary.py`, `wiki.py`, `_common.py`.
...
2. Generate (per-type async endpoint, 202 + job_id):
   ...
   - **database**: MCP introspection flow over the product's bound MCP servers → wiki-style DB docs → verification → indexing. Raw DSN is masked on acceptance and never stored.
   - **links**: storage only.
...
- **Docgen/expert/DB-RE**: `DOCGEN_MAX_WORKERS`, `DOCGEN_JUDGE_ENABLED`, `MERMAID_VERIFY`, `RLM_MODEL_CONTEXT_WINDOW` (legacy-named context-window knob), `DEEP_RESEARCH_TIMEOUT_SECONDS`, `DB_INTROSPECTION_TIMEOUT_SECONDS`.
