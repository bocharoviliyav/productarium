...
| **Entity Repository** | Single source for Product, Codebase, Spec, Links, Database DB queries; entity + per-page verification flags (server-owned; page flags reset on content change); `?light=1` shaping (`light_pages`/`strip_page_content`) + PUT round-trip body restore (`merge_stored_page_bodies`) | `api/repositories/product_repo.py` | `routers/products.py`, `routers/databases.py`, `docgen/` |
...
| **Doc Versioning (Vault-style)** | Immutable doc version snapshots (`productarium_doc_versions`), baseline bootstrap, append/restore, `current_version` pointers, version REST endpoints (detail/restore honor `?light=1`) | `api/repositories/doc_version_repo.py`<br>`api/routers/doc_versions.py` | `docgen/jobs.py`, `routers/products.py`, `routers/databases.py`, `routers/docgen.py` |
...
| **Frontend: Views** | Dashboard, Product details, Artifact docs viewer (light product fetch `?light=1` + lazy per-page body `pageDetail`), Admin panels | `src/app/page.tsx`<br>`src/app/products/[productId]/page.tsx`<br>`src/app/products/[productId]/artifacts/[artifactId]/page.tsx` | Next.js App Router |
...
6. **Light Payloads & Lazy Page Loading** (6 GB-tab fix): every Product-returning endpoint (GET/PUT products, add/update/delete/verify for codebases|specs|links|databases, doc-version restore) accepts `?light=1` — page-bearing entities keep only page metadata (`_PAGE_META_KEYS`: tree fields + verify flags) and drop `content`/`provenance` plus the redundant `generated_docs` blob; page-less legacy entities keep their blob. The viewer fetches the open page body via `GET /{product_id}/{codebases|databases}/{id}/pages/{page_id}[?version=N]` (`routers/docgen.py`, ro-access, `source: current|archive`); `update_product` re-fills omitted bodies from storage (`merge_stored_page_bodies`) so a light round-trip cannot wipe content; archived version detail honors `?light=1` the same way.
...
7. **Per-page / Branch Regeneration**: `POST .../pages/{page_id}/regenerate` (codebases/databases only) submits a job with `force_pages=[page_id]`, or with `branch: true` the page plus all descendants (parent-link BFS in `routers/docgen.py:_branch_page_ids`); codebase maps pages to `force_units` with judge-issue reviewer notes, database regenerates and merges only the forced pages into old `pages` (wholesale fingerprint reuse keeps stored content/provenance but takes tree fields — parent/title/… — from the fresh render).
