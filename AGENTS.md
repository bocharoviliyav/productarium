...
| **DocGen: Database (RE)** | MCP introspection walk (Oracle cross-schema `ALL_*` bulk walk first; PG system-schema/extension filtering; pack-authoritative categories), role classifier, SQL catalog packs, LLM enrichment | `api/docgen/database.py`<br>`api/docgen/introspection_cache.py` | `routers/databases.py`, `models.py:Database` |
...
3. **Generation Pipeline**:
   ...
   - **Database RE**: Outbound MCP walk (`api/docgen/database.py`): Oracle takes a cross-schema `ALL_*` bulk walk first (per-owner columns/comments, `BIN$` skipped), otherwise the per-schema adapter walk; system schemas/owners and extension-shipped objects (`pg_depend`) are excluded; SQL packs replace category listings authoritatively -> Catalog introspection -> Table/relation schemas -> Batched LLM enrichment (`DB_DOCGEN_ENRICH_BATCH`) -> ER diagrams.
