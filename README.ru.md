...
> ⚠️ **Этот перевод устарел.** После миграции на LangChain/LangGraph (удаление adalflow/cognee/fast-rlm, добавление MCP-платформы, pgvector-памяти, deepagents-генерации, Deep Research и реверс-инжиниринга баз данных) актуальная документация находится в [README.md](./README.md). Этот файл пока не обновлён и описывает старую архитектуру.
...
**Все тела промптов вынесены** в `refs/prompts/*.md` (контракты всех 7 секций вики — одним файлом `docgen_sections.md`; контракты подстраниц — `docgen_subpages.md`; планировщик декомпозиции — `docgen_decomposer.md`; роутер `docgen_router.md`, оркестратор `docgen_orchestrator.md`, системные/секционные промпты агентов, spec-доки, экспертный агент, итерации Deep Research). Редактируются напрямую — без изменения кода. Загружаются через `load_prompt_file()`.

Промпты на русском (рассчитаны на Qwen3.5-35b-a3b) с английскими техническими терминами. Структура секционного контракта (блоки `<section id="...">` в `docgen_sections.md`):
...
```mermaid
flowchart TD
  G([POST /codebases/.../generate]) --> CB[git clone --depth=1<br/>state dir]
  CB --> Brief[Фаза 0: repo brief<br/>file tree + языки + README<br/>без LLM]
  Brief --> Router[Фаза 1: роутер — 1 LLM-вызов<br/>подсказки files/focus по секциям]
  Router --> Decomp[Фаза 1.5: декомпозер — 1 LLM-вызов<br/>юниты: capabilities / APIs / stores]
  Decomp --> Notes[notes-workspace<br/>repo_brief + decomposition.json<br/>+ summary-заметки юнитов]
  Notes --> Orch[Фаза 2: deepagents-оркестратор<br/>юнит-сабагенты через task<br/>дети раньше родителя]
  Orch -->|нет юнитов| FB[автофолбэк: питон-параллель<br/>DOCGEN_SECTION_CONCURRENCY]
  Orch --> Out[7 секций + подстраницы]
  FB --> Out
  Out --> Ver[verification.py по каждому юниту<br/>цитаты + judge + Mermaid + diff]
  Ver --> Pers[_persist_artifact<br/>generated_docs + pages]
  Pers --> Idx[_index_in_background<br/>pgvector]
```

Каждый сабагент сам исследует клон (read-only тулы `repo_list_files`/`repo_read_file`/`repo_grep`, капы адаптируются под контекстное окно модели) и пишет ровно одну секцию — черновики соседних секций в промпт не подмешиваются. Для малых контекстных окон все капы масштабируются от резолвинга окна (`repo_read_file`, hints, инструкции, `max_tokens ≈ ctx/6`), промпты укладываются в реальный tiktoken-бюджет дропом ЦЕЛЫХ файловых блоков с конца, а переполнения контекста логируются с фактическими бюджетами.

Генерация per-type (без полиморфной диспетчеризации): `codebases/{id}/generate` вызывает `generate_codebase_docs` (оркестратор/сабагенты + фолбэки); `specs/{id}/generate` вызывает `generate_openapi_docs`/`generate_asyncapi_docs` (stdlib parse + skeleton + LLM enrichment); `databases/{id}/generate` — MCP-интроспекция. Links не генерируются. Все сохраняют `generated_docs` + `pages` и индексируют в pgvector.

7 секций генерируются изолированными сабагентами-юнитами (оркестратор + автофолбэк на питон-параллель). Функциональное описание, Технические детали и Модель данных — родительские страницы с подстраницами (сначала дети, затем родитель-агрегатор; лимиты: ≤10 возможностей, ≤12 технических юнитов, ≤6 слоёв данных). При diff-регенерации неизменённые юниты (родители и дети) переиспользуются дословно; изменившийся состав детей заставляет родителя регенерироваться:
1. **Общая информация** (Overview) — название, стек, ВСЕ возможности (вход для gap-анализа), требования, структура.
2. **Архитектура** (Architecture, C4).
3. **Функциональное описание** (Functional) — карта возможностей + подстраницы по каждой возможности (sequence/activity диаграммы бизнес-процессов).
4. **Технические детали** (Technical) — таблица API + подстраницы по каждому эндпоинту/джобе/интеграции/справочнику.
5. **CI/CD и SRE** — пайплайны/сборка/контейнеры/деплой + метрики, логи, трейсы, health-checks, алертинг, runbook.
6. **QA (Тестирование)** — фреймворки, таксономия тестов, фактическое покрытие, фикстуры, tooling.
7. **Модель данных** (Data Model) — обзор хранилищ + подстраницы по каждому слою данных.

Генерация асинхронная: бэкенд отдаёт `202 + job_id`, тяжёлая работа (git clone, агенты, верификация) выполняется в worker-потоке с прогресс-моделью (фазы queued → cloning → planning → sections → verifying → indexing → done, счётчик секций, tqdm-стиль в логах). Фронтенд опрашивает `/generate/status?job_id=` каждые 2 секунды (макс ~30 минут), показывает на карточке счётчик вида `3/7 · System Architecture`, а после перезагрузки страницы восстанавливает идущие генерации через `GET /docgen/active`.
...
- `refs/` — референсы (`current.json`) + `refs/prompts/*.md` (все тела промптов).
