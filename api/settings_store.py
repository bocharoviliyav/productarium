...
def get_model_for_task(task: str) -> Dict[str, Optional[str]]:
    ...
    # Task-specific env fallbacks.
    if provider == "ollama":
        default_base = os.environ.get("OLLAMA_HOST", "http://localhost:11434") + "/v1"
        default_model = os.environ.get("RLM_MODEL_NAME") or os.environ.get("LLM_MODEL") or "qwen3:8b"
        default_key = os.environ.get("LLM_API_KEY") or os.environ.get("LOCAL_OPENAI_API_KEY") or "not-needed"
    else:
        # openai_local: LM Studio / llama.cpp / vLLM. LOCAL_OPENAI_BASE_URL
        # points at the OpenAI-compatible server (default LM Studio :1234).
        default_base = os.environ.get("LOCAL_OPENAI_BASE_URL", "http://localhost:1234/v1")
        default_model = os.environ.get("LOCAL_OPENAI_MODEL") or os.environ.get("RLM_MODEL_NAME") or "qwen/qwen3.6-27b"
...
def get_rlm_mode(task: str) -> str:
    ...
    ``api.rlm_runner``), ALWAYS returns ``llm`` so callers never try RLM when
    it cannot work — this is the "guaranteed operation" baseline.
    """
    # Check fast-rlm availability WITHOUT importing rlm_runner at module load
    # (rlm_runner imports fast_rlm lazily; we read its flag defensively).
    try:
        from api.rlm_runner import _FAST_RLM_AVAILABLE  # lazy; avoids circular import
