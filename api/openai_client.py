...
class OpenAIClient(ModelClient):
    ...
    Args:
        api_key (Optional[str], optional): OpenAI-compatible API key. Defaults to `None`.
        ...
        base_url (str): The API base URL to use when initializing the client.
            Defaults to `"http://localhost:8080/v1"` via the ``LOCAL_OPENAI_BASE_URL`` env var,
            but can be customized for third-party API providers or self-hosted models.
        env_api_key_name (str): The environment variable name for the API key. Defaults to `"LOCAL_OPENAI_API_KEY"`.
    ...
    def __init__(
        ...
        env_base_url_name: str = "LOCAL_OPENAI_BASE_URL",
        env_api_key_name: str = "LOCAL_OPENAI_API_KEY",
    ):
        r"""Initialize OpenAI client for local or remote API endpoints.

        For local endpoints (llama.cpp, vLLM, text-generation-webui, etc.),
        the API key is optional. Use 'not-needed' as a placeholder if required.

        Args:
            api_key (Optional[str], optional): OpenAI API key. Defaults to None.
            base_url (str): The API base URL. Defaults to LOCAL_OPENAI_BASE_URL env var or http://localhost:8080/v1.
            env_api_key_name (str): The environment variable name for the API key. Defaults to `"LOCAL_OPENAI_API_KEY"`.
        """
        super().__init__()
