...
# Embedder settings - default to the OpenAI-compatible local server (LM
# Studio exposes nomic-embed-text via its /v1/embeddings endpoint). Ollama is
# still supported when DEEPWIKI_EMBEDDER_TYPE=ollama is set explicitly.
...
def load_embedder_config():
    ...
    for key in ["embedder_ollama", "embedder_openai_local"]:
...
def get_embedder_config():
    ...
    if embedder_type == 'openai_local' and 'embedder_openai_local' in configs:
        ...
        # Default to ollama embedder for fully local operation
        return configs.get("embedder_ollama", {})
...
def is_openai_local_embedder():
    """
    Check if the current embedder configuration uses local OpenAI-compatible API.

    Returns:
        bool: True if using local OpenAI embedder, False otherwise
    """
    embedder_config = get_embedder_config()
    if not embedder_config:
        return False

    # Check if model_client is OpenAIClient (local)
    model_client = embedder_config.get("model_client")
    if model_client:
        return model_client.__name__ == "OpenAIClient"

    # Fallback: check client_class string
    client_class = embedder_config.get("client_class", "")
    return client_class == "OpenAIClient"
...
def get_embedder_type():
    ...
    Returns:
        str: 'ollama' (default) or 'openai_local'
    """
    if is_openai_local_embedder():
        return 'openai_local'
    else:
        return 'ollama'
...
if embedder_config:
    for key in ["embedder_ollama", "embedder_openai_local", "retriever", "text_splitter"]:
...
def get_model_config(provider="ollama", model=None):
    ...
    Parameters:
        provider (str): Model provider ('ollama' or 'openai_local')
...
def fetch_ollama_models():
    ...
    Returns:
        list: List of available model names with their details
    """
    import requests
...
def fetch_openai_local_models(base_url: str = None):
    """
    Fetch available models from local OpenAI-compatible API.
...
def get_available_models():
    ...
    This function dynamically fetches models from Ollama and OpenAI-compatible APIs.
