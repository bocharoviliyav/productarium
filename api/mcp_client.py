"""Productarium Local MCP (Model Context Protocol) Client Interface.

Allows Productarium services (Expert Agent, background tasks, API endpoints) to
discover and invoke tools from all configured local and remote MCP servers
(http/SSE and stdio subprocess transports).

Uses `McpConnector` from `api.integrations.mcp` as the underlying transport engine.
"""
