...
class RAG(adal.Component):
    ...
    def __init__(self, provider=None, model=None, use_s3: bool = False, base_url: str = None, api_key: str = None, embedding_model: str = None):  # noqa: F841 - use_s3 is kept for compatibility
        """
        Initialize the RAG component.

        Args:
            provider: Model provider to use ('openai_local' or 'ollama');
                defaults to the DEEPWIKI_DEFAULT_PROVIDER env var ('openai_local').
            model: Model name to use with the provider
            use_s3: Whether to use S3 for database storage (default: False)
            base_url: Custom base URL for the model provider
            api_key: Custom API key for the model provider
            embedding_model: Custom model for embeddings
        """
        super().__init__()
        ...
        # Update config if custom embedding model is provided
        if self.embedding_model:
            from api.config import configs
            if self.provider == "ollama" and "embedder_ollama" in configs:
                configs["embedder_ollama"]["model_kwargs"]["model"] = self.embedding_model
            elif self.provider == "openai_local" and "embedder_openai_local" in configs:
                configs["embedder_openai_local"]["model_kwargs"]["model"] = self.embedding_model
        ...
        if self.is_ollama_embedder:
            ...
            from api.config import get_embedder_config, OLLAMA_HOST
            
            embedder_config = get_embedder_config()
            if embedder_config and embedder_config.get("model_kwargs", {}).get("model"):
                model_name = embedder_config["model_kwargs"]["model"]
                exists, error_msg, available_models = check_ollama_model_exists(model_name)
                if not exists:
                    if available_models:
                        raise Exception(f"Ollama model '{model_name}' not found. Available models: {available_models}. Run 'ollama pull {model_name}' to install it.")
                    else:
                        raise Exception(f"Cannot connect to Ollama at {OLLAMA_HOST}. Please ensure Ollama is running. Error: {error_msg}")
