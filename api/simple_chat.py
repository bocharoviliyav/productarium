...
async def chat_completions_stream(request: ChatCompletionRequest):
    ...
    try:
        ...
        if request.provider == "ollama":
            ...
            # Propagate seed for deterministic generation when configured.
            if "seed" in model_config:
            ...
            logger.warning(f"Unknown provider '{request.provider}', defaulting to Ollama")
            prompt += " /no_think"
            ...
            if "seed" in model_config:
                _def_options["seed"] = model_config["seed"]
            model_kwargs = {
                "model": model_config.get("model", "qwen3:8b"),
                "stream": True,
            ...
            try:
                if request.provider == "ollama" or (request.provider not in ["ollama", "openai_local"]):
                    ...
                    try:
                        ...
                        response = await model.acall(api_kwargs=api_kwargs, model_type=ModelType.LLM)
                        # Handle streaming response from local OpenAI API
                        async for chunk in response:
                        ...
                        logger.error(f"Error with local OpenAI API: {str(e_openai)}")
                        yield f"\nError with local OpenAI API: {str(e_openai)}\n\nPlease check that your local OpenAI-compatible server is running at the configured endpoint."
                    ...
                    try:
                        ...
                        if request.provider == "ollama" or request.provider not in ["ollama", "openai_local"]:
                            ...
                            try:
                                ...
                                # Get the response using the simplified prompt
                                logger.info("Making fallback local OpenAI API call")
                                fallback_response = await model.acall(api_kwargs=fallback_api_kwargs, model_type=ModelType.LLM)

                                # Handle streaming fallback_response from local OpenAI API
                                async for chunk in fallback_response:
                                    choices = getattr(chunk, "choices", [])
                                    if len(choices) > 0:
                                        delta = getattr(choices[0], "delta", None)
                                ...
                                logger.error(f"Error with local OpenAI API fallback: {str(e_fallback)}")
                                yield f"\nError with local OpenAI API fallback: {str(e_fallback)}\n\nPlease check that your local OpenAI-compatible server is running."
